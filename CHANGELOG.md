<img src="brand/logo.svg" alt="Bin Collection" style="max-width: 750px;">

If you find this project useful, consider supporting its development:

<a href="https://www.buymeacoffee.com/hunter.nl" target="_blank"><img src="https://cdn.buymeacoffee.com/buttons/v2/default-yellow.png" alt="Buy Me A Coffee" style="height: 60px !important;width: 217px !important;"></a>

# Changelog
## [v1.0.3](https://github.com/hunter-nl/HA-Bin-Collection/releases/tag/v1.0.3)


### 🐛 Fixes
- [Fix: simplify card service selector](https://github.com/hunter-nl/HA-Bin-Collection/pull/37) ([#37](https://github.com/hunter-nl/HA-Bin-Collection/pull/37))

## [v1.0.2](https://github.com/hunter-nl/HA-Bin-Collection/releases/tag/v1.0.2)


### 🐛 Fixes
- [Fix: trim changelog template whitespace](https://github.com/hunter-nl/HA-Bin-Collection/pull/35) ([#35](https://github.com/hunter-nl/HA-Bin-Collection/pull/35))

## [v1.0.1](https://github.com/hunter-nl/HA-Bin-Collection/releases/tag/v1.0.1)


### 🐛 Fixes
- [Fix: use service name on card](https://github.com/hunter-nl/HA-Bin-Collection/pull/33) ([#33](https://github.com/hunter-nl/HA-Bin-Collection/pull/33))
- [Fix: show card startup placeholder](https://github.com/hunter-nl/HA-Bin-Collection/pull/32) ([#32](https://github.com/hunter-nl/HA-Bin-Collection/pull/32))

## [v1.0.0](https://github.com/hunter-nl/HA-Bin-Collection/releases/tag/v1.0.0)


### ✨ Features
- [Feat: add generic provider diagnostic script](https://github.com/hunter-nl/HA-Bin-Collection/pull/24) ([#24](https://github.com/hunter-nl/HA-Bin-Collection/pull/24))
- [Feat: add dashboard collection card](https://github.com/hunter-nl/HA-Bin-Collection/pull/14) ([#14](https://github.com/hunter-nl/HA-Bin-Collection/pull/14))

### ❓ Miscellaneous
- [## Summary](https://github.com/hunter-nl/HA-Bin-Collection/pull/6) ([#6](https://github.com/hunter-nl/HA-Bin-Collection/pull/6))

### 🐛 Fixes
- [Fix: apply card options explicitly](https://github.com/hunter-nl/HA-Bin-Collection/pull/30) ([#30](https://github.com/hunter-nl/HA-Bin-Collection/pull/30))
- [Fix: compact logging options](https://github.com/hunter-nl/HA-Bin-Collection/pull/29) ([#29](https://github.com/hunter-nl/HA-Bin-Collection/pull/29))
- [Fix: show card startup state](https://github.com/hunter-nl/HA-Bin-Collection/pull/27) ([#27](https://github.com/hunter-nl/HA-Bin-Collection/pull/27))
- [Fix: parse ACV calendar data list](https://github.com/hunter-nl/HA-Bin-Collection/pull/26) ([#26](https://github.com/hunter-nl/HA-Bin-Collection/pull/26))
- [Fix: persist card editor changes](https://github.com/hunter-nl/HA-Bin-Collection/pull/23) ([#23](https://github.com/hunter-nl/HA-Bin-Collection/pull/23))
- [Fix: use current ACV Ximmio API](https://github.com/hunter-nl/HA-Bin-Collection/pull/21) ([#21](https://github.com/hunter-nl/HA-Bin-Collection/pull/21))
- [Fix: localize card labels](https://github.com/hunter-nl/HA-Bin-Collection/pull/18) ([#18](https://github.com/hunter-nl/HA-Bin-Collection/pull/18))
- [Fix: localize calendar event labels](https://github.com/hunter-nl/HA-Bin-Collection/pull/19) ([#19](https://github.com/hunter-nl/HA-Bin-Collection/pull/19))
- [Fix: move provider diagnostics out of logs](https://github.com/hunter-nl/HA-Bin-Collection/pull/20) ([#20](https://github.com/hunter-nl/HA-Bin-Collection/pull/20))
- [Fix: improve card notice display](https://github.com/hunter-nl/HA-Bin-Collection/pull/16) ([#16](https://github.com/hunter-nl/HA-Bin-Collection/pull/16))
- [Fix: retain recent pickup notices](https://github.com/hunter-nl/HA-Bin-Collection/pull/13) ([#13](https://github.com/hunter-nl/HA-Bin-Collection/pull/13))
- [Fix: address beta review findings](https://github.com/hunter-nl/HA-Bin-Collection/pull/11) ([#11](https://github.com/hunter-nl/HA-Bin-Collection/pull/11))
- [Fix: log collection fetches and parse provider data](https://github.com/hunter-nl/HA-Bin-Collection/pull/10) ([#10](https://github.com/hunter-nl/HA-Bin-Collection/pull/10))
- [Fix: show providers in a dropdown](https://github.com/hunter-nl/HA-Bin-Collection/pull/9) ([#9](https://github.com/hunter-nl/HA-Bin-Collection/pull/9))
- [Fix: serialize config flow schema](https://github.com/hunter-nl/HA-Bin-Collection/pull/5) ([#5](https://github.com/hunter-nl/HA-Bin-Collection/pull/5))
- [Fix filename for HA Bin Collection](https://github.com/hunter-nl/HA-Bin-Collection/pull/4) ([#4](https://github.com/hunter-nl/HA-Bin-Collection/pull/4))
- [Update README for HA Bin Collection clarity](https://github.com/hunter-nl/HA-Bin-Collection/pull/2) ([#2](https://github.com/hunter-nl/HA-Bin-Collection/pull/2))

### ⬆️ Dependencies
- [Chore(deps):(deps): bump the dependencies group with 2 updates](https://github.com/hunter-nl/HA-Bin-Collection/pull/1) ([#1](https://github.com/hunter-nl/HA-Bin-Collection/pull/1))



## ❤️ New Contributors

* @releases-manager[bot] made their first contribution
in [#31](https://github.com/hunter-nl/HA-Bin-Collection/pull/31)

* @hunter-nl made their first contribution
in [#30](https://github.com/hunter-nl/HA-Bin-Collection/pull/30)

* @github-actions[bot] made their first contribution
in [#1](https://github.com/hunter-nl/HA-Bin-Collection/pull/1)

* @dependabot[bot] made their first contribution

* @ made their first contribution## Funding

If you find this project useful, consider supporting its development:

<a href="https://www.buymeacoffee.com/hunter.nl" target="_blank"><img src="https://cdn.buymeacoffee.com/buttons/v2/default-yellow.png" alt="Buy Me A Coffee" style="height: 60px !important;width: 217px !important;"></a>
