class HaBinCollectionCard extends HTMLElement {
  setConfig(config) {
    if (!config.entity) throw new Error("Specify an HA Bin Collection overview entity.");
    this.config = config;
  }
  ...
  render() {
    ...
    if (!state) { this.innerHTML = `<ha-card><div class="empty">Entity not found: ${this.config.entity}</div></ha-card>`; return; }
    ...
    const icon = (type) => `<img class="bin ${type}" src="/ha_bin_collection/${({rest: "Kliko_rest", paper: "Kliko_paper", gft: "Kliko_gft", pmd: "Kliko_pmd"}[type] || "None")}.png" alt="">`;
    const rows = collections.length ? collections.map(item => `<div class="pickup">${icon(item.type)}<span>${labels[item.type] || item.source_type}</span><time>${item.date}</time></div>`).join("") : `<div class="empty">No upcoming collections</div>`;
    const messages = notices.map(notice => `<div class="notice"><b>${notice.title}</b><br>${notice.body}</div>`).join("");
...
HaBinCollectionCard.prototype.constructor = HaBinCollectionCard;
customElements.define("ha-bin-collection-card", HaBinCollectionCard);
window.customCards = window.customCards || [];
window.customCards.push({type: "ha-bin-collection-card", name: "HA Bin Collection Card", description: "Upcoming waste collections and collector notices."});
